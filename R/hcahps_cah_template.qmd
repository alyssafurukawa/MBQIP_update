...
response_data <- rbind(c("state",
                  "# Completed Surveys", prettyNum(unique(state_data[state_data$period==8,]$st_denom), big.mark = ","), " ", "Response Rate", round_perc(unique(state_data[state_data$period==8,]$st_resp_rate)/100), " ") ,
                 c("nat", 
                  "# Completed Surveys", prettyNum(unique(state_data[state_data$period==8,]$nat_denom), big.mark = ","), " ", "Response Rate", round_perc(unique(state_data[state_data$period==8,]$nat_resp_rate)/100), " ")) %>%
  as_tibble() |> 
  mutate(state_label = glue::glue(""))
...
#table_label_cah <- "Your Hospital's \nAdjusted Score \n"
table_label_cah <- glue::glue("Your Hospital's \nAdjusted Score \n{range8}")
                   
table_label_state <- "Your State's \nCAH Data \n" 
table_label_us <- "National CAH Data \n"
...
composite_table <- function(x) {
  ...
  tab_spanner(
    ...
    tab_spanner(
      label = glue::glue("
                         {response_data[response_data$V1=='state',]$V5}
                         {response_data[response_data$V1=='state',]$V6}"),
      columns = c(st_rate_sometimes , st_rate_usually , st_rate_always) ,
      id = "response_pct_state"
    ) |> 
    tab_spanner(
      label = glue::glue("{response_data[response_data$V1=='state',]$V2}
                         {response_data[response_data$V1=='state',]$V3}"),
      spanners = "response_pct_state" ,
      id = "response_n_state"
    ) |> 
  tab_spanner(
    label = md(glue::glue("**Your State's CAH Data**")) ,
    #label = md(glue::glue("**{table_label_state} {range8}**")),
    #columns = c(st_rate_sometimes , st_rate_usually , st_rate_always) ,
    spanners =  "response_n_state" ,
    id = "state_id"
  ) |> 
     tab_spanner(
      label = glue::glue("{response_data[response_data$V1=='nat',]$V5}
                         {response_data[response_data$V1=='nat',]$V6}"),
      #columns = c(nat_rate_sometimes , nat_rate_usually , nat_rate_always) ,
      columns = c(nat_rate_sometimes , nat_rate_usually , nat_rate_always) ,
      id = "response_pct_nat"
    ) |> 
     tab_spanner(
      label = glue::glue("{response_data[response_data$V1=='nat',]$V2}
                         {response_data[response_data$V1=='nat',]$V3}"),
      columns = c(nat_rate_sometimes , nat_rate_usually , nat_rate_always) ,
      spanners = "response_pct_nat" , 
      id = "response_n_nat"
    ) |> 
  tab_spanner(
    label = md(glue::glue("**National CAH Data**")) ,
    #label = md(glue::glue("**{table_label_us} {range8}**")),
    #columns = c(nat_rate_sometimes , nat_rate_usually , nat_rate_always) ,
    spanners = "response_n_nat" ,
    id = "nat_id"
  ) |> 
  tab_spanner(
  ...
  tab_style(
   ...
   tab_style(
     ...
     # locations = cells_column_spanners(c("hsp_id" , "state_id" , "nat_id"))
...
```{r setup composite 1 figure , include=FALSE , echo=FALSE , warning=FALSE }
...
for (i in hcahps_measures){
  ...
  geom_text(data= filter(figure_data, cah_state == "Your Hospital"), 
            aes(x= period, y= est, label= rnd_perc(est, dec=F),
                vjust= FMT2::vjust_hcahps2(figure_data, upper=.91)  ## 2024-05-31: the vjust_hcahps2 function can cause problems sometimes, usually a list in the CAH because of duplicates but I do not know how they appear tbh, troubleshoot at some point -af 
                ), 
            size=4) +
