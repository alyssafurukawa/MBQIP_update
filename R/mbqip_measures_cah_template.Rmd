  ...
  if (!(per %in% cah$period)){
      ...
      if(per==4) {
        st_nt <- state_data %>% #get state and natl data
          filter(period==4) %>%
          slice(1) %>%
          select(period, edtc_st_denom:nt_alledtc_rate , state_name) ##adding edtc_st_denom 
        
        st_agg <- cah %>% #get state 4Q aggregate 
          filter(period== max_per) %>%
          select(provider_id:alledtc_mou_allq_rate) %>%
          mutate(period=4) %>%
          mutate(across(c(denom:alledtc, quarter:edtc_report), ~NA)) %>% ## 2024-06-26: updated here with edtc_report
...
for (per in periods){
  ...
  if ("measure_4" %in% colnames(edtc_temp)) {
    ...
    assign("denom_st", cah %>% filter(period==4) %>% select(edtc_st_denom))
    ...
    if (!(per %in% cah_temp$period)){
        ...
        st_nt <- state_data %>% #get state and natl data
            filter(period==per, #changed ==4 to ==per, 11/23/21
                   measure==m) %>%
            slice(1) %>%
            select(measure, cah_state, period, opip_st_denom:state_name) %>% #added cah_state 1/26/21
          ## 2024-06-26: added opip_st_denom instead of st_denom 
          mutate(provider_id= as.numeric(i)) #added 11/23/21
        
        cah <- cah %>% bind_rows(st_nt)
    }
  }
}
    ...
    if (!(per %in% cah_temp$period)){
        ...
        st_nt <- state_data %>% #get state and natl data
            filter(period==per, #for now nothing will be added for ABS p3; changed ==3 to ==per, 11/23/21
                   measure==m) %>%
            slice(3) %>%
            select(provider_id, cah_state, measure, period, opip_st_denom:state_name) %>% #added cah_state here 1/26/21
          mutate(provider_id= as.numeric(i)) #added 11/23/21
...
ami_right <- cah %>%
  ...
  select(measure, opip_st_report, st_num, st_p90, nat_report, nat_num, benchmark) #changed nat p90 to ben 11/19
