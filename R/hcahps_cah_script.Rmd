...
p2_q <- "2022Q4"
p1_q <- "2022Q3"
lastQ_data <- "MBQIP Reports/Quarterly Reports/HCAHPS/2023 Q1/R/Data/hcahps_2023Q1_2023-10-25.Rdata"
#^^^^^^ manually update for every report ^^^^^^^^^^^^^^^^^


#MOU list, updated to most recent 10_25_23
list <- readxl::read_xlsx(paste0(fmt_folder, "MBQIP Reports/CAH List with MOU_FMT 1.3.24.xlsx"), sheet = "CAHs with MOU") %>%
  clean_names() %>%
...
closed <- c(281321 , 421303 , 151335 , 451345 , 451360 , 441311 ,371324 ,
231317 , 
191303 ,
171340 ,
151302  ) 
...
hcahps_raw <- read_hcahps("2023/HCAHPS/Q2 2023/HRSA_April_2024_reporting.xlsx", joinMOU= TRUE)


hcahps_outcomes <-  readxl::read_xlsx(paste0(fmt_folder, "Data/MBQIP Data/2023/HCAHPS/Q2 2023/HRSA_outcomes_April_2024_reporting.xlsx")) %>% 
...
hcahps_trim <- hcahps_cah %>% 
  filter(!(period==4 & provider_id %in% nomou_p4), #remove CAHs w/o MOUs
...
## 7 dec 22: don't have to create the directory every time obvs -af 
...
report_period <- "Current Reporting Period: Q3 2022 - Q2 2023"
...
range4 <- "Q3 2022 - Q2 2023"
range3 <- "Q2 2022 - Q1 2023"
range2 <- "Q1 2022 - Q4 2022"
range1 <- "Q4 2021 - Q3 2022" 
...
for (st in states){
  ...
  for (i in cah_list){
    ...
    count <- cah %>% filter(period==4) %>% distinct(completed_surveys) %>% unlist(use.names = FALSE)
    #maybe add some rounding for 1000+
   new_count <- cah %>% filter(period==4) %>% distinct(ps_flag) %>% unlist(use.names = FALSE)
   count2 <- ifelse(new_count== 1, "*", count) ## 28 aug 23: updated to try to replace NA with * when appropriate using the ps_flag
    cah_resp <- cah %>% filter(period==4) %>% distinct(resp_rate) %>% unlist(use.names = FALSE)
    new_cah_resp<- cah %>% filter(period==4) %>% distinct(ps_flag) %>% unlist(use.names = FALSE)
    ...
    #rmarkdown::render(#input = paste0(fmt_folder, "MBQIP Reports/Quarterly Reports/HCAHPS/", folder, "/R/HCAHPS CAH Report Template.Rmd"),
                      ...
                      output_format = "pdf_document",
                      output_file = paste0(st, "_", cah_id,"_", name, "_HCAHPS_", p4,".pdf"),
